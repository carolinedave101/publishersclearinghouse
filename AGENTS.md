...
- **CSV export per link**: "Export CSV" button on the link view page → `RegistrationLink::exportWinnersCsv()` (chunked, proper quoting) → `response()->streamDownload`; test `tests/Feature/RegistrationLinkCsvExportTest.php` (4 tests)
...
- **Bug fixed 2026-08-19:** view page 500 — relation manager "View Winner" action called `WinnerResource::getUrl('view')` (no view page exists; only index/create/edit) → changed to `getUrl('edit')`
- **Bug fixed 2026-08-19:** `/winner/claim` threw 500 — `Table 'notifications' doesn't exist` (Laravel 11 skeleton ships no notifications migration, but `WinnerClaimedNotification` sends via DatabaseChannel). Added `2026_08_19_130000_create_notifications_table` (standard Laravel schema; `morphs()` already creates the composite index — don't add a duplicate). Test: `tests/Feature/WinnerClaimTest.php` (2 tests; uses `$admin->id` not hardcoded 1 — IDs vary in full-suite runs). Verified live: claim → 302 → dashboard, notifications rows stored for all admin users
