    ...
    <div class="max-w-7xl mx-auto px-4">
        <div class="flex items-center justify-between h-16">
            <a href="{{ route('home') }}" class="flex items-center gap-2 group">
                <img src="{{ $siteConfig['logo'] ?? asset('logo.png') }}" alt="{{ $siteConfig['site_name'] ?? 'Publishers Clearing House' }}" class="h-9 w-auto group-hover:scale-105 transition-transform" style="border-radius: 10px;">
            </a>
            <div class="hidden md:flex items-center gap-6">
                <a href="{{ route('home') }}" class="text-sm text-white/70 hover:text-[#D4AF37] transition-colors">Home</a>
                <a href="{{ route('giveaways') }}" class="text-sm text-white/70 hover:text-[#D4AF37] transition-colors">Giveaways</a>
                <a href="{{ route('games') }}" class="text-sm text-white/70 hover:text-[#D4AF37] transition-colors">Games</a>
                <a href="{{ route('shop') }}" class="text-sm text-white/70 hover:text-[#D4AF37] transition-colors relative">
                    Shop
                ...
                <a href="{{ route('memberships') }}" class="text-sm text-white/70 hover:text-[#D4AF37] transition-colors">Memberships</a>
                @if(session('winner_id'))
                    @php
                        $winnerNav = \App\Models\Winner::find(session('winner_id'));
                    @endphp
                    <a href="{{ route('winner.dashboard') }}" class="hidden lg:flex items-center gap-2 text-sm bg-[#D4AF37]/10 hover:bg-[#D4AF37]/20 text-white px-4 py-2 rounded-lg border border-[#D4AF37]/20 hover:border-[#D4AF37]/40 transition-all">
                        <span>🏠</span>
                    ...
                    <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                        <button @click="open = !open" class="flex items-center gap-2 text-sm text-white/70 hover:text-[#D4AF37] transition-colors bg-white/5 hover:bg-[#D4AF37]/10 px-3 py-2 rounded-lg border border-white/10 hover:border-[#D4AF37]/30 transition-all">
                            ...
                            <span class="font-medium">{{ $winnerNav?->first_name }}</span>
                        ...
                        <div x-show="open" x-cloak class="absolute right-0 top-full mt-2 w-72 bg-white rounded-xl shadow-2xl border border-[#D4AF37]/20 py-2 z-50">
                            <div class="px-4 py-3 border-b border-gray-100">
                                ...
                                <p class="text-sm font-bold text-[#1B2A4A]">{{ $winnerNav?->first_name }} {{ $winnerNav?->last_name }}</p>
                                <p class="text-[10px] text-[#D4AF37] font-mono mt-0.5">Code: {{ $winnerNav?->unique_code }}</p>
                            </div>
                            <div class="py-1">
                                <a href="{{ route('winner.dashboard') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-[#1B2A4A] hover:bg-[#D4AF37]/5 font-semibold transition-colors">
                            ...
                            <div class="px-4 py-2">
                                <form method="POST" action="{{ route('winner.logout') }}">
                                    @csrf
        ...
        <div x-show="mobileOpen" x-cloak class="md:hidden pb-4">
            <div class="flex flex-col gap-2">
                <a href="{{ route('home') }}" class="text-sm text-white/70 hover:text-[#D4AF37] py-1">Home</a>
                <a href="{{ route('giveaways') }}" class="text-sm text-white/70 hover:text-[#D4AF37] py-1">Giveaways</a>
                <a href="{{ route('games') }}" class="text-sm text-white/70 hover:text-[#D4AF37] py-1">Games</a>
                <a href="{{ route('shop') }}" class="text-sm text-white/70 hover:text-[#D4AF37] py-1">
                    Shop <span x-show="$store.cart.count > 0" x-text="'(' + $store.cart.count + ')'" x-cloak class="text-[#D4AF37]"></span>
                </a>
                <a href="{{ route('memberships') }}" class="text-sm text-white/70 hover:text-[#D4AF37] py-1">Memberships</a>
                @if(session('winner_id'))
                    <a href="{{ route('winner.dashboard') }}" class="text-sm bg-[#D4AF37]/20 text-[#D4AF37] font-bold py-2 px-3 rounded-lg border border-[#D4AF37]/30">🏠 My Dashboard</a>
                    <form method="POST" action="{{ route('winner.logout') }}">
                        @csrf
