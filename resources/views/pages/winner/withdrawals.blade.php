...
@section('title', 'Withdraw Funds - PCH Winners Portal')
...
<div class="bg-gradient-to-r from-[#1B2A4A] to-[#0F1A2E] border-b border-[#D4AF37]/10">
    <div class="max-w-6xl mx-auto px-4 py-4">
        <div class="flex items-center gap-2 text-xs text-white/40 mb-3">
            ...
            <span>/</span>
            <a href="{{ route('winner.dashboard') }}" class="hover:text-[#D4AF37] transition-colors">Dashboard</a>
            <span>/</span>
            <span class="text-[#D4AF37]">Withdraw Funds</span>
        </div>
        <div class="flex flex-wrap items-center gap-2">
            <a href="{{ route('winner.dashboard') }}" class="px-4 py-2 bg-white/5 text-white/70 rounded-lg text-sm font-medium border border-white/10 hover:text-[#D4AF37] hover:border-[#D4AF37]/30">Dashboard</a>
            @if($winnerConfig['show_deposits'])
            <a href="{{ route('winner.deposits') }}" class="px-4 py-2 bg-white/5 text-white/70 rounded-lg text-sm font-medium border border-white/10 hover:text-[#D4AF37] hover:border-[#D4AF37]/30">Make Deposit</a>
            @endif
            <a href="{{ route('winner.withdrawals') }}" class="px-4 py-2 bg-[#D4AF37]/20 text-[#D4AF37] rounded-lg text-sm font-semibold border border-[#D4AF37]/30">Withdraw Funds</a>
            @if($winnerConfig['show_transactions'])
            <a href="{{ route('winner.transactions') }}" class="px-4 py-2 bg-white/5 text-white/70 rounded-lg text-sm font-medium border border-white/10 hover:text-[#D4AF37] hover:border-[#D4AF37]/30">Transaction History</a>
            @endif
        </div>
    </div>
</div>
    ...
    <div class="max-w-4xl mx-auto px-4 py-8">
        ...
        <div class="grid md:grid-cols-3 gap-6 mb-8">
            <div class="bg-white rounded-2xl gold-border p-6 text-center">
                ...
                <p class="text-2xl font-bold gold-text-gradient">${{ number_format($winner->prize_amount, 0) }}</p>
            </div>
            <div class="bg-white rounded-2xl gold-border p-6 text-center">
                <div class="text-3xl mb-2">💳</div>
                <p class="text-xs text-gray-400 mb-1">Total Withdrawn</p>
                <p class="text-2xl font-bold text-red-500">${{ number_format($winner->withdrawals->whereIn('status', ['approved', 'completed'])->sum('amount'), 0) }}</p>
            </div>
            <div class="bg-white rounded-2xl gold-border p-6 text-center">
                <div class="text-3xl mb-2">✅</div>
                <p class="text-xs text-gray-400 mb-1">Available Balance</p>
                <p class="text-2xl font-bold text-green-600">${{ number_format($winner->available_balance, 0) }}</p>
            </div>
        </div>
        @endif
            ...
            <div class="bg-white rounded-2xl gold-border p-6 mb-8">
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-400">Available balance: ${{ number_format($winner->available_balance, 0) }}</p>
                    </div>
                </div>

                <form action="{{ route('winner.withdrawals.request') }}" method="POST" class="space-y-4" x-data="withdrawalForm()">
                    @csrf
                    ...
                    <div>
                        ...
                        <select name="payment_method_id" x-model="methodId" @change="loadMethodDetails()" required
                            ...
                            @foreach($paymentMethods as $method)
                                <option value="{{ $method->id }}" data-instructions="{{ $method->instructions }}">{{ $method->name }} - {{ ucfirst($method->type) }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="amount" min="1" max="{{ $winner->available_balance }}" step="0.01" required
                                placeholder="0.00"
                        ...
                        <p class="text-xs text-gray-400 mt-1">Max: ${{ number_format($winner->available_balance, 2) }}</p>
                        @error('amount')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
        ...
        @else
            <div class="bg-white rounded-2xl gold-border p-8 text-center mb-8">
                ...
                <a href="{{ route('winner.dashboard') }}" class="inline-block mt-4 px-6 py-2.5 bg-gradient-to-r from-[#D4AF37] to-[#B8960F] text-[#1B2A4A] rounded-xl text-sm font-bold">Go to Dashboard</a>
            </div>
        @endif
        ...
        <div class="bg-white rounded-2xl gold-border overflow-hidden">
            <div class="p-5 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-[#D4AF37]/5 to-transparent">
                ...
                <span class="text-xs bg-[#D4AF37]/10 text-[#D4AF37] px-3 py-1 rounded-full font-semibold">{{ $winner->withdrawals->count() }}</span>
            </div>
            <div class="overflow-x-auto">
                @forelse($winner->withdrawals->sortByDesc('created_at') as $withdrawal)
                    <div class="p-4 border-b border-gray-100 last:border-0 hover:bg-gray-50/50 transition-colors">
                        <div class="flex items-center justify-between mb-2">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-sm font-semibold text-[#1B2A4A]">${{ number_format($withdrawal->amount, 2) }}</p>
                                    <p class="text-xs text-gray-400">{{ $withdrawal->paymentMethod?->name ?? 'Unknown' }}</p>
                                </div>
                            </div>
                            <span class="text-xs px-2.5 py-1 rounded-full font-medium {{ $statusColors[$withdrawal->status] ?? 'bg-gray-100' }}">
                                {{ ucfirst($withdrawal->status) }}
                            </span>
                        </div>
                        <div class="flex items-center justify-between">
                            @if($winnerConfig['show_dates'])
                                <p class="text-[10px] text-gray-400">{{ $withdrawal->created_at->format('M j, Y g:i A') }}</p>
                            @endif
                            @if($withdrawal->admin_notes)
                                <p class="text-[10px] text-gray-400 italic truncate max-w-[200px]">Note: {{ strip_tags($withdrawal->admin_notes) }}</p>
                            @endif
                        </div>
                    </div>
                @empty
...
function withdrawalForm() {
    return {
        ...
        methodInstructions: '',
        account: {},
        methods: @json($paymentMethods->mapWithKeys(fn($m) => [$m->id => ['type' => $m->type, 'instructions' => $m->instructions]])),
        loadMethodDetails() {
