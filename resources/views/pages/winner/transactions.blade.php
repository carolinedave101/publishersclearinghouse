...
@section('title', 'Transaction History - PCH Winners Portal')
...
<div class="bg-gradient-to-r from-[#1B2A4A] to-[#0F1A2E] border-b border-[#D4AF37]/10">
    <div class="max-w-6xl mx-auto px-4 py-4">
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            @endif
            <a href="{{ route('winner.transactions') }}" class="px-4 py-2 bg-[#D4AF37]/20 text-[#D4AF37] rounded-lg text-sm font-semibold border border-[#D4AF37]/30">Transaction History</a>
        </div>
    </div>
</div>
    ...
    <div class="max-w-4xl mx-auto px-4 py-8">
        ...
        <div class="grid md:grid-cols-4 gap-4 mb-8">
            <div class="bg-white rounded-2xl gold-border p-5 text-center">
                ...
                <p class="text-xl font-bold gold-text-gradient">${{ number_format($winner->prize_amount, 0) }}</p>
            </div>
            <div class="bg-white rounded-2xl gold-border p-5 text-center">
                <p class="text-xs text-gray-400 mb-1">Total Deposits</p>
                <p class="text-xl font-bold text-green-600">${{ number_format($winner->deposits->where('status', 'approved')->sum('amount'), 0) }}</p>
            </div>
            <div class="bg-white rounded-2xl gold-border p-5 text-center">
                <p class="text-xs text-gray-400 mb-1">Total Withdrawn</p>
                <p class="text-xl font-bold text-red-500">${{ number_format($winner->withdrawals->whereIn('status', ['approved', 'completed'])->sum('amount'), 0) }}</p>
            </div>
            <div class="bg-white rounded-2xl gold-border p-5 text-center">
                <p class="text-xs text-gray-400 mb-1">Available Balance</p>
                <p class="text-xl font-bold text-green-600">${{ number_format($winner->available_balance, 0) }}</p>
            </div>
        </div>
        @endif
                        ...
                        <tbody>
                            @foreach($allTransactions as $tx)
                                <tr class="border-t border-gray-100 hover:bg-gray-50/50 transition-colors">
                                    @if($winnerConfig['show_dates'])
                                    <td class="px-4 py-3 text-xs text-gray-500 whitespace-nowrap">{{ $tx['date']->format('M j, Y g:i A') }}</td>
                                    @endif
                                    <td class="px-4 py-3">
                                        <span class="flex items-center gap-2 text-sm">
                                            <span>{{ $tx['icon'] }}</span>
                                            <span class="font-medium text-[#1B2A4A]">{{ $tx['type'] }}</span>
                                        </span>
                                    </td>
                                    <td class="px-4 py-3 text-xs text-gray-500">{{ $tx['method'] }}</td>
                                    <td class="px-4 py-3 text-sm font-medium {{ $tx['color'] }}">${{ number_format(abs($tx['amount']), 2) }}</td>
                                    <td class="px-4 py-3 text-xs text-gray-400">${{ number_format($tx['fee'], 2) }}</td>
                                    <td class="px-4 py-3 text-sm font-semibold {{ $tx['net'] >= 0 ? 'text-green-600' : 'text-red-500' }}">
                                        {{ $tx['net'] >= 0 ? '+' : '' }}${{ number_format($tx['net'], 2) }}
                                    </td>
                                    <td class="px-4 py-3">
                                        <span class="text-xs px-2.5 py-1 rounded-full font-medium
                                            @switch($tx['status'])
                                                @case('pending') bg-yellow-100 text-yellow-700 @break
                                                @case('approved') bg-blue-100 text-blue-700 @break
