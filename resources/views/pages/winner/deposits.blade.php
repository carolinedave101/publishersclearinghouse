...
@section('title', 'Make a Deposit - PCH Winners Portal')
...
<div class="bg-gradient-to-r from-[#1B2A4A] to-[#0F1A2E] border-b border-[#D4AF37]/10">
    <div class="max-w-6xl mx-auto px-4 py-4">
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            <a href="{{ route('winner.deposits') }}" class="px-4 py-2 bg-[#D4AF37]/20 text-[#D4AF37] rounded-lg text-sm font-semibold border border-[#D4AF37]/30">Make Deposit</a>
            @if($winnerConfig['show_withdrawals'])
            <a href="{{ route('winner.withdrawals') }}" class="px-4 py-2 bg-white/5 text-white/70 rounded-lg text-sm font-medium border border-white/10 hover:text-[#D4AF37] hover:border-[#D4AF37]/30">Withdraw Funds</a>
            @endif
    ...
    <div class="max-w-4xl mx-auto px-4 py-8">
        ...
        <div class="grid md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="bg-white rounded-2xl gold-border p-6 text-center">
                ...
                <p class="text-2xl font-bold text-green-600">${{ number_format($winner->deposits->where('status', 'approved')->sum('amount'), 0) }}</p>
            </div>
            <div class="bg-white rounded-2xl gold-border p-6 text-center">
                <div class="text-3xl mb-2">✅</div>
        ...
        <div class="bg-white rounded-2xl gold-border p-6 mb-8">
            ...
            <form action="{{ route('winner.deposits.submit') }}" method="POST" enctype="multipart/form-data" class="space-y-4" x-data="depositForm()">
                @csrf
        ...
        <div class="bg-white rounded-2xl gold-border overflow-hidden">
            <div class="p-5 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-[#D4AF37]/5 to-transparent">
                ...
                <span class="text-xs bg-[#D4AF37]/10 text-[#D4AF37] px-3 py-1 rounded-full font-semibold">{{ $winner->deposits->count() }}</span>
            </div>
            <div class="overflow-x-auto">
                @forelse($winner->deposits->sortByDesc('created_at') as $deposit)
                    <div class="p-4 border-b border-gray-100 last:border-0 hover:bg-gray-50/50 transition-colors">
                        <div class="flex items-center justify-between mb-2">
                            <div class="flex items-center gap-3">
                                ...
                                @endphp
                                <span>{{ $statusIcons[$deposit->status] ?? '📄' }}</span>
                                <div>
                                    <p class="text-sm font-semibold text-[#1B2A4A]">${{ number_format($deposit->amount, 2) }}</p>
                                    <p class="text-xs text-gray-400">{{ $deposit->paymentMethod?->name ?? 'Unknown' }}</p>
                                </div>
                            </div>
                            <span class="text-xs px-2.5 py-1 rounded-full font-medium {{ $statusColors[$deposit->status] ?? 'bg-gray-100' }}">
                                {{ ucfirst($deposit->status) }}
                            </span>
                        </div>
                        <div class="flex items-center justify-between">
                            @if($winnerConfig['show_dates'])
                                <p class="text-[10px] text-gray-400">{{ $deposit->created_at->format('M j, Y g:i A') }}</p>
                            @endif
                            @if($deposit->admin_notes)
                                <p class="text-[10px] text-gray-400 italic truncate max-w-[200px]">Note: {{ strip_tags($deposit->admin_notes) }}</p>
                            @endif
                        </div>
                    </div>
                @empty
...
function depositForm() {
    return {
        methodId: '',
        methodInstructions: '',
        methods: @json($paymentMethods->mapWithKeys(fn($m) => [$m->id => ['instructions' => $m->instructions]])),
        loadMethodDetails() {
