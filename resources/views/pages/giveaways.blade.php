...
@section('title', 'Giveaways - PCH Winners Portal')
    ...
    <div class="max-w-6xl mx-auto px-4 -mt-10 pb-16">
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6" id="giveaways-container">
            @foreach($giveaways as $giveaway)
                <div class="giveaway-card winner-card-hover bg-white rounded-2xl gold-border overflow-hidden" data-status="{{ $giveaway['status'] }}" data-id="{{ $giveaway['id'] }}">
                    ...
                    <div class="p-5">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-xs font-semibold px-2.5 py-0.5 rounded-full
                                @if($giveaway['status'] === 'active') bg-green-100 text-green-700
                                @elseif($giveaway['status'] === 'upcoming') bg-blue-100 text-blue-700
                                @else bg-gray-100 text-gray-600 @endif">
                                {{ ucfirst($giveaway['status']) }}
                            </span>
                            <span class="text-sm font-bold gold-text-gradient">{{ $giveaway['prizeValue'] }}</span>
                        </div>
                        <h3 class="font-bold text-[#1B2A4A] text-lg mb-1">{{ $giveaway['title'] }}</h3>
                        <p class="text-sm text-gray-500 mb-3 line-clamp-2">{{ $giveaway['description'] }}</p>
                        <div class="flex items-center justify-between text-xs text-gray-400 mb-3">
                            <span>⏱ Ends in <span class="countdown" data-ends="{{ $giveaway['endsAt'] }}">--</span></span>
                            <span class="entry-count-label" data-id="{{ $giveaway['id'] }}">{{ number_format($giveaway['entries']) }}/{{ number_format($giveaway['maxEntries']) }}</span>
                        </div>
                        <div class="w-full bg-gray-100 rounded-full h-2 mb-4">
                            <div class="bg-gradient-to-r from-[#D4AF37] to-amber-500 h-2 rounded-full entry-bar" data-id="{{ $giveaway['id'] }}" style="width: {{ min(100, $giveaway['maxEntries'] > 0 ? ($giveaway['entries'] / $giveaway['maxEntries']) * 100 : 0) }}%"></div>
                        </div>
                        @if($giveaway['status'] === 'active')
                            <button onclick="openEntryForm({{ $giveaway['id'] }}, '{{ addslashes($giveaway['title']) }}')" class="enter-btn w-full py-2.5 bg-gradient-to-r from-[#D4AF37] to-[#B8960F] text-[#1B2A4A] rounded-xl text-sm font-bold hover:from-[#C5A55A] hover:to-[#A8850D] transition-all shadow-lg shadow-[#D4AF37]/20">
                                🎯 Enter Now
                            </button>
                        @else
...
document.getElementById('entryForm').addEventListener('submit', async function(e) {
    ...
    try {
        const res = await fetch('{{ url("/giveaways") }}/' + currentGiveawayId + '/enter', {
            method: 'POST',
            headers: {
